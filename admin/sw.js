const CACHE_NAME = 'lazmall-admin-v1';

// App-shell files: always fetched fresh from the network first (falls back to
// cache only when offline), so a new deploy reaches admins immediately — no
...
// ---------- Real Web Push ----------
// ---------- Real Web Push ----------
// Fires even when the admin panel is fully closed. `silent: false` (default, set
// explicitly for clarity) makes the OS play its normal notification sound;
// `vibrate` adds a vibration pattern too on devices that support it. No way to play
// a custom sound file here — only the browser/OS's own system notification sound.
